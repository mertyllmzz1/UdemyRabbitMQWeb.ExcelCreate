...
namespace FileCreateWorkerService
{
    public class Worker : BackgroundService
    {
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            multipartFormDataContent.Add(new ByteArrayContent(ms.ToArray()),"file",Guid.NewGuid().ToString());

            var baseUrl = "https://localhost:44360/api/files";
            ...
            using(var httpClient = new HttpClient())
            {
               var response =  await httpClient.PostAsync($"{baseUrl}?fileId={createExcelMessage.FileId}", multipartFormDataContent);
                ...
                {
                    _logger.LogInformation($"Excel file created for {createExcelMessage.FileId}");
